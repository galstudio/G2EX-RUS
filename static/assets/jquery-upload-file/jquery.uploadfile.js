...
(function ($) {
    if($.fn.ajaxForm == undefined) {
        $.getScript(("https:"==document.location.protocol?"https://":"http://")+"7xsrj3.com1.z0.glb.clouddn.com/js/jquery.form.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            onCancel: function (files, pd) {return false;},
            onAbort: function (files, pd) {return false;},
            ...
            dragDropStr: "",
            uploadStr:"Изображение",
            abortStr: "中止",
            cancelStr: "Отмена",
            deletelStr: "Удалить",
            doneStr: "Готово",
            multiDragErrorStr: "Загрузка не поддерживает перетаскивание фала.",
            extErrorStr: "Допустимые расширения: ",
            duplicateErrorStr: "Файл уже существует.",
            sizeErrorStr: "Ограничение на размер : ",
            uploadErrorStr: "В загрузке отказано!",
            maxFileCountErrorStr: "Ограничение количества:",
            downloadStr: "Загрузить",
            customErrorKeyStr: "jquery-upload-file-error",
            showQueueDiv: false,
            statusBarWidth: false,
            dragdropWidth: 400,
            showPreview: true,
            previewHeight: "auto",
            previewWidth: "100%",
       ...
       this.startUpload = function () {
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                ...
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if(s.showError) $("<div class='" + s.errorClass + "'>" + files[i].name + " " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
...
//          this.statusbar = $("<div class='ajax-file-upload-statusbar'></div>").width(s.statusBarWidth);
            ...
            this.abort.addClass("ajax-file-upload-red").hide();
            this.done.addClass("ajax-file-upload-green").hide();
            this.download.addClass("ajax-file-upload-green").hide();
            this.cancel.addClass("ajax-file-upload-red").hide();
            this.del.addClass("ajax-file-upload-red").hide();
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span class='" + s.errorClass + "'> " + msg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
