...
class Comment extends ActiveRecord
{
    ...
    public function getComment()
    {
        return $this->hasOne(self::className(), ['id' => 'id'])
            ->select(['id', 'created_at', 'user_id', 'position','vote_count', 'invisible', 'content']);
    }
...
public static function updateCounterInfo($action, $id)
    {
        $upd = [
            'followVote_comment' => ['vote_count'=>1],
            'unfollowVote_comment' => ['vote_count'=>-1],
        ];
    ...
    public static function getCommentsFromView($topic_id, $pages)
    {
        ...
        if ( intval($settings['cache_enabled']) === 0 || ($models = $cache->get($key)) === false ) {
            ...
            if ( !empty($pids) ) {
                $models = static::find()->select(['id', 'position', 'user_id', 'created_at','vote_count', 'invisible', 'good', 'content'])
                    ->where(['topic_id' => $topic_id, 'position'=>$pids])
                    ->orderBy(['position'=>SORT_ASC])
                    ->with(['author','author.userInfo'])
                    ->limit($pages->limit)
                    ->asArray()
                    ->all();
            } else {
