...
//            ['status', 'default', 'value' => self::STATUS_ACTIVE],
    ...
    public function getTopics()
    {
        return $this->hasMany(Topic::className(), ['user_id' => 'id'])
            ->select(['id', 'node_id', 'user_id', 'reply_id', 'title','views','alltop','top', 'created_at','comment_count','vote_count', 'good','star','replied_at'])
    ...
    public function isAppendMore($append_count)
    {
        return ( $append_count > 2 );
    }

    public function isAppendExpired($created_at)
    {
        return ( time() < $created_at + intval(Yii::$app->params['settings']['edit_space'])*60 );
    }
    ...
    public function canAppend($model, $status=0)
    {
        return ( $status == 0 && self::isActive()
                 && self::isAuthor($model['user_id'])
                 //&& !self::isAppendMore($model['append_count'])
                 //&& !self::isAppendExpired($model['created_at'])
                );
    }
    ...
    {
        $costs = [
            'adminreg' => 10000000,  //Администратор
            'reg' => 100,  //Регистрация
            'mission' => 1000,  //Первая задача
            'addTopic' => -20,  //Добавление топика
            'addComment' => -5, //Добавление комментария
            'deleteTopic' => -20,  //Удаление топика
            'deleteComment' => -5, //Удаление комментария
            'commented' => 5,   //Прокомментировали
            'sendMsg' => -5,    //Отправить ПС
            'buyInviteCode' => -50, //Покупка инвайта
            'signin_10days' => 200, //Вход в систему 10 дней подряд
            'signin' => function() {return rand(10, 50);},  //Бонус посещаемости случайное число 10-50
            'thanks' => -20, //Поблагодарить
        ];
        if ($action === 'signin') {
            return $costs[$action]();
        } else {
...
*/
    ...
    public function afterAddAppend($cost, $append)
    {
        $this->updateScore($cost);
        $author_id = $append->topic->user_id;
        if ($this->id != $author_id) {
            $author = static::findOne($author_id);
                (new History([
                    'user_id' => $author_id,
    ...
    public function mission()
        {
        $action = $this->getLastAction(History::ACTION_ORIGINAL_SCORE)->one();
        $cost = static::getCost('mission');
        $now = time();
        $this->updateScore($cost);
        (new History([
            'user_id' => $this->id,
            'type' => History::TYPE_POINT,
            'action' => History::ACTION_ORIGINAL_SCORE,
            'action_time' => $now,
            'ext' => json_encode(['score'=>$this->score, 'cost'=>$cost]),
        ]))->save(false);
    ...
    {
        $types = [
            'topic' => Notice::TYPE_THANK_TOPIC,
            'comment' => Notice::TYPE_THANK_COMMENT,
        ];
        $actions = [
        ...
        if (!$target) {
            ...
            return ['result'=>0, 'msg'=>'Нельзя благодарить самого себя'];
        }
