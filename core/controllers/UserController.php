...
class UserController extends AppController
{
    public function actionView($username)
    {
        ...
        if ($req->getIsAjax()) {
            $this->layout = false;
            $model = $this->findUserModel($username, ['userInfo', 'topics.node', 'topics.lastReply', 'comments.topic.author','comments.topic.node']);
    ...
    {
        $model = User::find()->select(['id', 'created_at', 'status', 'username','reg', 'avatar', 'score', 'comment','role','avatar','email','created_at','updated_at'])->where(['username'=>$username]);
        if ( !empty($with) ) {
        ...
        if ($model !== null) {
            ...
            throw new NotFoundHttpException('会员 ['.$username.'] 未找到。');
        }
    }
