...
//              $attr['sourceName'] = $client->defaultName();
    ...
    public function actionSignup()
    {
        if ( intval($this->settings['close_register']) === 1) {
            return $this->render('opResult', ['title'=>'用户注册已关闭', 'status'=>'message', 'msg'=>'请使用'. \yii\helpers\Html::a('第三方帐号登录', ['site/login'])]);
        }
            ...
            if ($user = $model->signup()) {
                if (Yii::$app->getUser()->login($user)) {
                    if ($user->status == User::STATUS_INACTIVE) {
                        return $this->render('opResult', ['title'=>'Вы успешно зарегистрировались', 'status'=>'message', 'msg'=>'Спасибо за регистрацию, подтверждение регистрации было отправлено на ваш почтовый адрес.']);
                    } else if ($user->status == User::STATUS_ADMIN_VERIFY) {
                        return $this->render('opResult', ['title'=>'Регистрация аккаунта pfdthityf', 'status'=>'message', 'msg'=>'Благодарим Вас за регистрацию, пожалуйста, дождитесь подтверждения администратора.']);
                    } else {
                        return $this->goHome();
                    }
                }
    ...
    public function actionForgotPassword()
    {
        try {
            ...
            return $this->render('opResult', ['title'=>'密码重置申请', 'status'=>'problem', 'msg'=>$e->getMessage()]);
        }
        if ($model->load(Yii::$app->getRequest()->post()) && $model->validate()) {
            try {
                $model->apply();
                return $this->render('opResult', ['title'=>'密码重置申请', 'status'=>'message', 'msg'=>'一封包含了重设密码指令的邮件已经发送到你的注册邮箱，按照邮件中的提示，即可重设你的密码。']);
            } catch (InvalidParamException $e) {
    ...
    public function actionResetPassword($token)
    {
        try {
            ...
            return $this->render('opResult', ['title'=>'密码重置失败', 'status'=>'problem', 'msg'=>$e->getMessage()]);
        }

        if ($model->load(Yii::$app->getRequest()->post()) && $model->validate() && $model->resetPassword()) {
            return $this->render('opResult', ['title'=>'密码重置成功', 'status'=>'message', 'msg'=>'新密码已生效']);
        }
    ...
    public function actionVerifyEmail($token)
    {
        try {
            ...
            return $this->render('opResult', ['title'=>'邮箱绑定失败', 'status'=>'problem', 'msg'=>$e->getMessage()]);
        }
        ...
        if ( Yii::$app->getRequest()->getIsPost() ) {
            ...
            if ( !$model->validate() ) {
                 $result = ['title'=>'邮箱绑定失败', 'status'=>'problem', 'msg'=>'您申请绑定的邮箱['.$token->ext.']已被注册使用'];
            } else {
                ...
                $result = ['title'=>'邮箱绑定成功', 'status'=>'message', 'msg'=>'新邮件绑定成功'];
            }
        } else {
                $result = [
                    'title'=>'确认绑定邮箱',
    ...
    public function actionActivate($token)
    {
        try {
            ...
            return $this->render('opResult', ['title'=>'帐号激活失败', 'status'=>'problem', 'msg'=>$e->getMessage()]);
        }
        ...
        if ( Yii::$app->getRequest()->getIsPost() ) {
            ...
            if ( !empty($token->ext) && $user->email !== $token->ext && User::findOne(['email'=>$token->ext]) ) {
                return $this->render('opResult', ['title'=>'帐号激活失败', 'status'=>'problem', 'msg'=>'申请绑定邮箱['.$token->ext.']已被注册使用']);
            }
            if (intval($this->settings['admin_verify']) === 1) {
                $user->status = User::STATUS_ADMIN_VERIFY;
                $result = ['title'=>'注册邮箱确认成功', 'status'=>'message', 'msg'=>'注册邮箱确认成功，请等待管理员验证。'];
            } else {
                $user->status = User::STATUS_ACTIVE;
                $result = ['title'=>'帐号激活成功', 'status'=>'message', 'msg'=>'帐号激活成功，现在可以 '. \yii\helpers\Html::a('登录', ['site/signin'],['class' => 'super normal button']) .' 发贴和回帖了。'];
            }
            $user->email = $token->ext;
            $user->save(false);
